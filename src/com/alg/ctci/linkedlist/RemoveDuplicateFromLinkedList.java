...
public class RemoveDuplicateFromLinkedList{
	...
	/**
	We can store the LinkedList nodes in a hash table and
	check if the entry has already been made, if so, simply update
	the links of the duplicate entry, else we just make an entry in the
	hash table.
	We can just start with the first node (header node) and the rest
	...
	*/
	...
	/**
	In this method we attempt to remove the duplicates without
	using the buffer to store the intermediate result.
	We can use two pointers: one is a current pointer which iterates
	throughout the linked list and another is a runner pointer which checks all subsequent nodes for duplicates.
