...
public class StringCompression {
    ...
    /**
     * compress the string by using the count of same chars in adjacent
     * position. If the compressed string is not smaller than the original one,
    ...
    static String compressString(String a) {
        ...
        for (int i = 0; i < a.length(); i++) {
            ...
            } else //now if the previous string is same as current one, increase the count
            if (a.charAt(i) == a.charAt(i - 1)) {
                ...
                if (duplicateCount > 0) {
                    compressedString.append(String.valueOf(duplicateCount + 1)); //one for the first one and the rest for the duplicates
        ...
        return (compressedString.length() < a.length() ? compressedString.toString() : a);
    }
