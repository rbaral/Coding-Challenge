...
public class LinkedListAddForward{
	...
	*/
	public static PartialSum addNodesHelper(Node n1, Node n2){
		...
		sum.sum = insertBefore(sum.node, val%10);
		sum.carry = val/10;
		return sum;
	}
	...
	public static Node addNodesRecursive(Node n1, Node n2){
		...
		if(resultNode.carry>0){
			resultNode.sum = insertBefore(resultNode.node, resultNode.carry);
		}
	...
	public static void main(String args[]){
		...
		Node n21 = new Node(2);
		n21.next = new Node(3);
		n21.next.next = new Node(5);
		printLinkedList(n21);
		//Node sumNode = addNumbers1(linkedList1, linkedList2);
		Node sumNode = addNodesRecursive(n11, n21);
